...
### [Added] 纯 C# .NET 10 LTS + WinUI 3 + Windows App SDK 2.x 原生桌面端 UI 封装 (Native WinUI 3 Desktop UI)
- **全新原生技术栈工程 (`Linguist.WinUI/`)**：
  - 基于 **C# 14 / .NET 10 LTS**（`net10.0-windows10.0.19041.0`）与 **Windows App SDK 1.6+ / 2.x** 架构搭建现代 Windows 原生桌面工程；
  - 采用 **Unpackaged（非打包）** 独立部署模式 (`WindowsPackageType=None`)，免证书签名困扰，双击即秒速拉起运行；
  - 接入 **CommunityToolkit.Mvvm** 进行响应式 UI 数据绑定与状态分发。
- **Win32 API & DWM 深度底层互操作 (`Linguist.WinUI/Interop/Win32.cs`)**：
  - 封装 `DwmSetWindowAttribute`、`DWM_WINDOW_CORNER_PREFERENCE.DWMWCP_ROUND`、`DWMWA_BORDER_COLOR`、`DWMWA_SYSTEMBACKDROP_TYPE`；
  - 消除 Windows 11 默认 1px 矩形黑框与系统绘制硬边，原生支持 **32px 物理大圆角**；
  - 封装 `WM_NCLBUTTONDOWN` / `HTCAPTION` 与 `ReleaseCapture()`，实现桌面级零延迟、顺滑无抖动的拖拽移动体验。
- **悬浮翻译卡片 1:1 精准复刻 (`Linguist.WinUI/Views/CardView.xaml`, `CardView.xaml.cs`)**：
  - 严格保持 **390 × 520** 经典规格与 **32px 超大圆角**；
  - 苹果液态磨砂玻璃渐变（Apple Liquid Glass）与镜面高光外圈；
  - 顶部操作栏：三色交通灯控制点（清空/药丸收缩/刷新）、语言互换药丸（英语 ➔ 中文）、Gemini 引擎胶囊指示、截图翻译（Crop）、置顶固定（Pin）、字号比例胶囊（Type 92%）、偏好设置（Sliders）；
  - 主内容区：大圆角半透玻璃输入框与清空按钮；富词典释义模型（单词、音标 `[ɪˈfɪʃnt]`、发音、收藏红心）；蓝色词性胶囊（`adj.`）与分项释义；同义词气泡流（`effective`、`productive`、`streamlined`）；双语对照例句卡片；快捷操作工具栏（语言对、一键复制、发音、生词历史）；
  - 底部状态栏：Smart-Select 智能划词滑动胶囊开关、尺寸标示与缩放提示。
- **悬浮药丸胶囊 1:1 精准复刻 (`Linguist.WinUI/Views/PillView.xaml`, `PillView.xaml.cs`)**：
  - 严格保持 **46px 高度** 与 **9999px 全圆角胶囊** 构型；
  - 左侧微控双点（红清空、绿展开）；
  - 自适应搜索框（放大镜图标与输入控件）；
  - 原文与译文自适应对齐区（`Efficient` ➔ `高效的；有效率的；有能力的`）；
  - 右侧工具按钮群（发音、复制完整释义、展开为完整卡片）。
- **全要素 Windows UI Automation 无障碍支持**：
  - 在卡片与药丸所有交互控件中完整注入 `AutomationProperties.Name` 与 `AutomationProperties.AutomationId`，全面支持 Windows 辅助工具、自动化测试与屏幕阅读器。
- **完善的工程开发文档与 VS Code 开箱即用支持 (`Linguist.WinUI/README.md`, `.vscode/`)**：
  - 深度配置 `.vscode/launch.json`、`.vscode/tasks.json` 与 `.vscode/extensions.json`，在 VS Code 中可直接按 **F5** 或通过 `dotnet run` 零配置调试运行；
  - 涵盖 Visual Studio 2026/2022、VS Code 与 .NET CLI 命令行的一键编译、运行与发布指引。
...
- **前端 CSS 溢出裁切与四角暗影光晕消除 (`src/components/translator/FloatingTranslatorCard.tsx`, `src/index.css`)**：
  ...
  - 重构翻译历史抽屉 (`HistoryDrawer.tsx`) 与设置弹窗 (`SettingsModal.tsx`)：移除原有贴满窗口边缘的无圆角矩形结构，统一采用 `rounded-[32px]` / `rounded-[28px]` 超大圆角苹果液态磨砂玻璃 (`apple-liquid-glass backdrop-blur-3xl overflow-hidden`)，无论在主翻译界面还是打开生词本历史记录，全流程彻底告别直角矩形框；
